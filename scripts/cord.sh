...
helm repo update cord

# Get the list of all chart names in the cord repo
chart_names=$(helm search repo cord -l --output json | jq -r '.[].name')

# Get the list of all chart versions in the cord repo
chart_versions=$(helm search repo cord -l --output json | jq -r '.[].version')
...
for ((i=0; i<${#chart_names[@]}; i++)); do
  ...
  helm pull $chart_name --version $chart_version -d charts.opencord.ngkore.org
done
